...
function App() {
  ...
  const [purchesedPlayer, setPurchesedPlayer] = useState([])
   const removePlayer = (p) =>{
    const fileteredData = purchesedPlayer.filter(ply=> ply.player_id!==p.player_id)
  ...
  return (
    <>
      ...
      <div className="w-11/12 mx-auto flex justify-between items-center">
        <h1 className="font-bold text-2xl">{toggle?"Available Players":`Selected Player (${purchesedPlayer.length}/6)`}</h1>
      ...
      >
        <AvailablePlayers availableBlance={availableBlance} SetAvailableBlance={SetAvailableBlance} playerPromise={playerPromise} purchesedPlayer={purchesedPlayer} setPurchesedPlayer={setPurchesedPlayer}></AvailablePlayers>
